...
export function MetallicButton({
  ...
  const resolvedShellSize =
    shellSize ?? (isIcon ? "h-11.5 w-11.5" : "h-11.5 w-35.5");
  const resolvedFaceSize =
    faceSize ?? (isIcon ? "h-10.5 w-10.5" : "h-10.5 w-34.5");
  ...
  const actionClass =
    "absolute top-0 left-0 z-40 translate-z-6.25 cursor-pointer overflow-hidden rounded-full transform-3d no-underline outline-none";

  return (
    <div className={`relative inline-block ${className}`}>
      <div className="perspective-origin-[50%_50%] perspective-[1000px]">
        <div className={`relative transform-3d ${resolvedShellSize}`}>
          <div
            className={`pointer-events-none absolute top-0 left-0 z-30 flex translate-z-5 items-center justify-center gap-1.5 transform-3d ${resolvedShellSize}`}
          >
            {isIcon ? (
              <Sparkles
              ...
              <span
                className={`px-2 text-[13px] font-semibold uppercase whitespace-nowrap tracking-[0.02em] text-[#f4f8ff] [text-shadow:0px_1px_2px_rgba(0,0,0,0.5)] ${SETTLE}`}
              >
                {label}
              </span>
            )}
          ...
          <div
            className={`absolute top-0 left-0 z-20 translate-z-2.5 transform-3d ${resolvedShellSize} ${pressShift} ${SETTLE}`}
          >
            <div
              className={`m-0.5 rounded-full bg-[linear-gradient(180deg,#0E1B30_0%,#0A1424_100%)] ${resolvedFaceSize} ${SHADOW_SHIFT} ${isPressed ? FACE_PRESSED : "shadow-none"}`}
            />
          </div>

          <div
            className={`absolute top-0 left-0 z-10 translate-z-0 transform-3d ${resolvedShellSize} ${pressShift} ${SETTLE}`}
          >
            <div
              className={`rounded-full bg-transparent ${resolvedShellSize} ${SHADOW_SHIFT} ${
                isPressed
                  ? RIM_PRESSED
                  : isHovered
                    ? RIM_HOVERED
            ...
            >
              <div
                ref={surfaceRef}
                className={`metallic-button-canvas relative overflow-hidden rounded-full ${resolvedShellSize}`}
              />
            </div>
          </div>
          ...
          {href ? (
            <a
              ...
              aria-label={label}
              className={`${actionClass} ${resolvedShellSize}`}
            >
              {ripples.map((ripple) => (
                <span
                  key={ripple.id}
            ...
            <button
              ...
              onMouseUp={() => setIsPressed(false)}
              aria-label={label}
              className={`${actionClass} border-none bg-transparent outline-none ${resolvedShellSize}`}
            >
              {ripples.map((ripple) => (
                <span
                  key={ripple.id}
